...
//if the memory blocks for the two matrices are continuous we'll copy all the values in one shot
	if (dest_tda == dest_size2 && src_tda == src_size2){
		memcpy(dest->data, src->data, sizeof(ATOMIC) * src_size1 * src_size2);
		return GSL_SUCCESS;
	}
