...
namespace physx { namespace profile {
	...
	struct NullUserAllocator : public PxAllocatorCallback
	{
		PxU32 getHandle(const char*)
		{
			return 0;
		}
...
//			fprintf( stderr, "%s:%d: %s\n", file, line, message );
	...
	{
		mutable NullUserAllocator		mAllocator;
		...
		FoundationWrapper( PxAllocatorCallback* inUserAllocator = NULL  )
			: mUserAllocator( inUserAllocator )
		{
		}
