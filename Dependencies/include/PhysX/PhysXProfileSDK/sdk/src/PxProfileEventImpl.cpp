...
namespace physx { 
	...
	struct ProfileBulkEventHandlerBuffer
	{
		...
		void onEvent( const PxProfileEventId& inId, PxU32 threadId, PxU64 contextId, PxU8 cpuId, ThreadPriority::Enum threadPriority, PxU64 timestamp, EventTypes::Enum inType )
		{
			onEvent( Event( EventHeader( static_cast<PxU8>( inType ), inId.mEventId ), EventData( StartEvent( threadId, contextId, cpuId, static_cast<PxU8>( threadPriority ), timestamp ) ) ) );
		}
		void onStartEvent( const PxProfileEventId& inId, PxU32 threadId, PxU64 contextId, PxU8 cpuId, ThreadPriority::Enum threadPriority, PxU64 timestamp )
		{
			onEvent( inId, threadId, contextId, cpuId, threadPriority, timestamp, EventTypes::StartEvent );
		}
		void onStopEvent( const PxProfileEventId& inId, PxU32 threadId, PxU64 contextId, PxU8 cpuId, ThreadPriority::Enum threadPriority, PxU64 timestamp )
		{
		...
		{
			onEvent( Event( inId.mEventId, EventValue( value, contextId, threadId ) ) );
		}
		...
		{
			onEvent( Event( 0, CUDAProfileBuffer( submitTimestamp, timeSpan, cudaData, bufLenInBytes, bufferVersion ) ) );
		}
		void flush()
		{
			if ( mEventCount )
