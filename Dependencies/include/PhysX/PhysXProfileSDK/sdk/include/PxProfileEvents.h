...
namespace physx { namespace profile {
	...
	struct RelativeProfileEvent
	{
		...
		RelativeProfileEvent( PxU64 inTs ) : mTensOfNanoSeconds( inTs ) {}
	...
	{
		RelativeStartEvent( PxU64 inTs = 0 ) : RelativeProfileEvent( inTs ) {}
		...
		{
			inHdlr->onStartEvent( PxProfileEventId( eventId ), thread, context, inCpuId, static_cast<ThreadPriority::Enum>(threadPriority), mTensOfNanoSeconds );
		}
	};
	...
	{
		RelativeStopEvent( PxU64 inTs = 0 ) : RelativeProfileEvent( inTs ) {}
		template<typename THandlerType>
		void handle( THandlerType* inHdlr, PxU16 eventId, PxU32 thread, PxU64 context, PxU8 inCpuId, PxU8 threadPriority ) const
		{
			inHdlr->onStopEvent( PxProfileEventId( eventId ), thread, context, inCpuId, static_cast<ThreadPriority::Enum>(threadPriority), mTensOfNanoSeconds );
		}
	};
	...
	struct EventContextInformation
	{
		...
		EventContextInformation(PxU32 inThreadId = PX_MAX_U32
	...
	struct ProfileEvent
	{
		...
		ProfileEvent( PxU32 inThreadId, PxU64 inContextId, PxU8 inCpuId, PxU8 inPriority, PxU64 inTs )
			: mContextInformation( inThreadId, inContextId, inPriority, inCpuId )
			, mTimeData( inTs )
		{
		}
	...
	{
		StartEvent( PxU32 inThreadId = 0, PxU64 inContextId = 0, PxU8 inCpuId = 0, PxU8 inPriority = 0, PxU64 inTensOfNanoSeconds = 0 ) 
			: ProfileEvent( inThreadId, inContextId, inCpuId, inPriority, inTensOfNanoSeconds )
		{
		}
		RelativeStartEvent getRelativeEvent() const { return RelativeStartEvent( mTimeData.mTensOfNanoSeconds ); }
	...
	{
		StopEvent( PxU32 inThreadId = 0, PxU64 inContextId = 0, PxU8 inCpuId = 0, PxU8 inPriority = 0, PxU64 inTensOfNanoSeconds = 0 )
			: ProfileEvent( inThreadId, inContextId, inCpuId, inPriority, inTensOfNanoSeconds )
		{
		}
		RelativeStopEvent getRelativeEvent() const { return RelativeStopEvent( mTimeData.mTensOfNanoSeconds ); }
	...
	struct EventValue
	{
		...
		PxU64	mContextId;
		PxU32	mThreadId;
		EventValue( PxI64 inValue = 0, PxU64 inContextId = 0, PxU32 inThreadId = 0 )
	...
	struct CUDAProfileBuffer
	{
		...
		CUDAProfileBuffer( PxU64 timestamp = 0, PxF32 span = 0, const PxU8* cdata= 0, PxU32 buflen= 0, PxU32 version= 0 )
	...
	//Generic container that can contain all types that fit inside container.
	//One important aspect is that this object does a raw memcpy to place data
	//into its buffer.
	//Can support any simple type; but does not call the destructor of the type
	//upon destruction.
	struct EventData
	{
		PxU8 mDataBuffer[64];
		...
		template<typename TDataType>
		void bufferSizeCheck() { PX_ASSERT( sizeof( TDataType ) <= sizeof( mDataBuffer ) ); }

		template<typename TDataType>
		EventData( const TDataType& inData )
		{
			bufferSizeCheck<TDataType>();
			new (mDataBuffer) TDataType( inData );
		}
		EventData() { memset( mDataBuffer, 0, sizeof( mDataBuffer ) ); }

		template<typename TDataType>
		TDataType& toType() 
		{
			bufferSizeCheck<TDataType>();
			return *(reinterpret_cast<TDataType*>(mDataBuffer));
		}

		template<typename TDataType>
		const TDataType& toType() const { return const_cast<EventData*>(this)->toType<TDataType>();	}
	};
	...
	inline TRetVal visit( EventTypes::Enum inEventType, const EventData& inData, TOperator inOperator )
	{
		switch( inEventType )
		{
		case EventTypes::StartEvent:			return inOperator( inData.toType<StartEvent>() );
		case EventTypes::StopEvent:				return inOperator( inData.toType<StopEvent>() );
		case EventTypes::RelativeStartEvent:	return inOperator( inData.toType<RelativeStartEvent>() );
		case EventTypes::RelativeStopEvent:		return inOperator( inData.toType<RelativeStopEvent>() );
		case EventTypes::EventValue:			return inOperator( inData.toType<EventValue>() );
		case EventTypes::CUDAProfileBuffer:		return inOperator( inData.toType<CUDAProfileBuffer>() );
	...
	{
		EventData mData;
		EventDataEqualOperator( EventData inD ) : mData( inD ) {}
		template<typename TDataType> bool operator()( const TDataType& inRhs ) const { return mData.toType<TDataType>() == inRhs; }
	...
	public:
		Event() {}
		Event( EventHeader inHeader, const EventData& inData = EventData() )
			: mHeader( inHeader )
			, mData( inData )
		{
		}
		...
		template<typename TDataType>
		Event( PxU16 eventId, const TDataType& inType )
