...
namespace Sq
{
	...
	class StaticPruner : public Pruner
	{
		...
		PX_FORCE_INLINE	const Ice::AABBTree*	GetAABBTree()	const
												{
													// Lazy build
		...
		PX_FORCE_INLINE	const Ice::AABBTree*	HasAABBTree()	const	{ return mAABBTree;	}

		virtual			void					visualize(Cm::RenderOutput& out, PxU32 color);

//		virtual			bool					updateRecomputesPose()	const	{ return false;	}

		protected:
		// Acceleration structure
				Ice::AABBTree*					mAABBTree;		//!< AABB tree for static objects
