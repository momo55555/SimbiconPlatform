...
{
	// PX_SERIALIZATION
	setType(PxSerialType::eUSER_D6_JOINT);
	//~PX_SERIALIZATION
	D6JointData* data = reinterpret_cast<D6JointData*>(PX_ALLOC(sizeof(D6JointData)));
	mData = data;
...
PxQuat truncateSwing(const PxQuat& in, const PxVec3& twistAxis, PxReal shat, PxReal chat, bool& angularTrunc)
{
	...
	{
		twist.w = tw;
		twist.getImaginaryPart() = twistAxis*PxSqrt(1-tw*tw);
	}
...
void D6JointVisualize(PxRenderBuffer &out,
					  ...
					  PxU32 flags)
{
	...
	Cm::visualizeJointFrames(r, frameScale, cA2w, cB2w);
	...
	if(data.limited&TWIST_FLAG)
	{
		...
		Cm::visualizeAngularLimit(r, limitScale, cA2w, data.twistLimit.lower, data.twistLimit.upper, 
	...
	if(swing1Limited && swing2Limited)
	{
		...
		Cm::visualizeLimitCone(r, limitScale, cA2w, data.tqSwingZ, data.tqSwingY, 
	...
	else if(swing1Limited ^ swing2Limited)
	{
		...
		if(swing1Limited)
		{
			if(data.locked & SWING2_FLAG)
				Cm::visualizeAngularLimit(r, limitScale, cA2w * yToX, -data.swingLimit.yAngle, data.swingLimit.yAngle, 
					PxAbs(Ps::tanHalf(swing.y, swing.w)) > data.tqSwingY - data.tqSwingPad);
			else
				Cm::visualizeDoubleCone(r, limitScale, cA2w * zToX, data.swingLimit.yAngle, 
					PxAbs(tanHalfFromSin(aZ.dot(bX)))> data.thSwingY - data.thSwingPad);
		}
		else 
		{
			if(data.locked & SWING1_FLAG)
				Cm::visualizeAngularLimit(r, limitScale, cA2w * zToX, -data.swingLimit.zAngle, data.swingLimit.zAngle,
					PxAbs(Ps::tanHalf(swing.z, swing.w)) > data.tqSwingZ - data.tqSwingPad);
			else
				Cm::visualizeDoubleCone(r, limitScale, cA2w * yToX, data.swingLimit.zAngle,  
