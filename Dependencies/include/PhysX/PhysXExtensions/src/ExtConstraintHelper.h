...
namespace Ext
{
	namespace joint
	{
		...
		PX_INLINE PxQuat truncateAngular(const PxQuat& in, PxReal sinHalfTol, PxReal cosHalfTol, bool& truncated)
		{
			...
			PxQuat out;
			out.w = cosHalfTol;
			out.getImaginaryPart() = im * sinHalfTol * PxRecipSqrt(m);
			return out;
		}
		...
		public:
			...
			void prepareLockedAxes(const PxQuat& qA, const PxQuat& qB, const PxVec3& cB2cAp, PxU32 lin, PxU32 ang)
			{
				...
				if(ang)
				{
					...
					if(ang&1) angular(row[0], -2.0f*qB2qA.getImaginaryPart().x);
					if(ang&2) angular(row[1], -2.0f*qB2qA.getImaginaryPart().y);
					if(ang&4) angular(row[2], -2.0f*qB2qA.getImaginaryPart().z);
				}
