...
Ext::DefaultCpuDispatcher::DefaultCpuDispatcher(PxU32 numThreads, PxU32* affinityMasks)
	: mQueueEntryPool(EXT_TASK_QUEUE_ENTRY_POOL_SIZE), mNumThreads(numThreads), mShuttingDown(false)
{
	...
	mWorkerThreads = reinterpret_cast<CpuWorkerThread*>(PX_ALLOC(numThreads * sizeof(CpuWorkerThread)));
	if (mWorkerThreads)
	{
		...
		for(PxU32 i = 0; i < numThreads; ++i)
		{
			mWorkerThreads[i].start(Ps::Thread::DEFAULT_STACK_SIZE);
			if (affinityMasks)
