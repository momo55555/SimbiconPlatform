...
{
	bool overflow;
	PxU32 nbTouchedTris = Gu::MeshQuery::findOverlapTriangleMesh(geom, geomPose, triGeom, meshPose, mResultsMemory, mMaxNbResults, 0, overflow);
	...
	if(overflow)
	{
		...
		nbTouchedTris = Gu::MeshQuery::findOverlapTriangleMesh(geom, geomPose, triGeom, meshPose, mResultsMemory, mMaxNbResults, 0, overflow);
...
PxU32 PxFindOverlapTriangleMeshUtil::findOverlap(const PxGeometry& geom, const PxTransform& geomPose, const PxHeightFieldGeometry& hfGeom, const PxTransform& hfPose)
{
	...
	while(overflow)
	{
		nbTouchedTris = Gu::MeshQuery::findOverlapHeightField(geom, geomPose, hfGeom, hfPose, mResultsMemory, mMaxNbResults, 0, overflow);
		if(overflow)
		{
