...
namespace physx { namespace repx {
	...
	{
		writer.write( inPropName, createRepXObject( inDatatype, inMap.getIdForLiveObject( getBasePtr( inDatatype ) ) ) );
	}
	...
	inline void writePxVec3( PxStream& inStream, const PxVec3& inVec ) { inStream << inVec; }
	...
	template<typename TDataType>
	inline void BasicDatatypeWrite( PxStream& inStream, const TDataType& item ) { inStream << item; }
	...
	struct RepXVisitorWriter<PxArticulation> : RepXVisitorWriterBase<PxArticulation>
	{
		...
		void writeArticulationLink( const PxArticulationLink* inLink )
		{
			...
			gotoTopName();
			const TArticulationLinkLinkMap::Entry* theParentPtr = mArticulationLinkParents.find( inLink );
			if ( theParentPtr != NULL )
				writeProperty( mWriter, mIdMap, mTempBuffer, "Parent", reinterpret_cast<const void*>( theParentPtr->second ) );
			writeProperty( mWriter, mIdMap, mTempBuffer, "Id", reinterpret_cast<const void*>( inLink ) );
		...
		void handleArticulationLinks( const PxArticulationLinkCollectionProp& inProp )
		{
			...
			if ( count )
			{
				...
				for ( PxU32 idx = 0; idx < count; ++idx )
				{
					...
					if ( mArticulationLinkParents.find( theLink ) == NULL )
