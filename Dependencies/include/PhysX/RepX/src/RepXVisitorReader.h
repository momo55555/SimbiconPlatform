...
namespace physx { namespace repx {
	...
	inline TObjType* readReference( RepXReader& inReader, RepXIdToRepXObjectMap& inMap )
	{
		void* theData;
		char* theValue = const_cast<char*>( inReader.getCurrentItemValue() );
		strto( theData, theValue );
		return reinterpret_cast<TObjType*>( const_cast<void*>( inMap.getLiveObjectFromId( theData ).mLiveObject ) );
	}
	template<typename TObjType>
	inline TObjType* readReference( RepXReader& inReader, RepXIdToRepXObjectMap& inMap, const char* pname )
	{
		void* temp;
		if ( readProperty ( inReader, pname, temp ) )
			return reinterpret_cast<TObjType*>( const_cast<void*>( inMap.getLiveObjectFromId( temp ).mLiveObject ) );
		return NULL;
	}
		...
		if ( visitor.gotoTopName() )
		{
					...
					success = visitor.gotoNextSibling() )
			{
				...
				if ( visitor.gotoChild( "Materials" ) )
				{
					...
					{
						materials.pushBack( readReference<PxMaterial>( theReader, idMap ) );
					}
				}
				...
				if ( visitor.gotoChild( "Geometry" ) )
				{
					if ( visitor.gotoFirstChild() )
					{
						...
						if ( physx::string::stricmp( geomTypeName, "PxSphereGeometry" ) == 0 ) result = parseGeometry(visitor, sphere);
						else if ( physx::string::stricmp( geomTypeName, "PxPlaneGeometry" ) == 0 ) result = parseGeometry(visitor, plane);
						else if ( physx::string::stricmp( geomTypeName, "PxCapsuleGeometry" ) == 0 ) result = parseGeometry(visitor, capsule);
						else if ( physx::string::stricmp( geomTypeName, "PxBoxGeometry" ) == 0 ) result = parseGeometry(visitor, box);
						else if ( physx::string::stricmp( geomTypeName, "PxConvexMeshGeometry" ) == 0 ) result = parseGeometry(visitor, convex);
						else if ( physx::string::stricmp( geomTypeName, "PxTriangleMeshGeometry" ) == 0 ) result = parseGeometry(visitor, mesh);
						else if ( physx::string::stricmp( geomTypeName, "PxHeightFieldGeometry" ) == 0 ) result = parseGeometry(visitor, heightField);
						else
							PX_ASSERT( false );
					}
				}
	...
	struct RepXVisitorReaderBase
	{
		...
		TRefType* readReference()
		{
			return physx::repx::readReference<TRefType>( mReader, mIdMap, topName() );
		}
		...
		{
			outProp = readReference<PxConvexMesh>();
			return true;
		}
		
		inline bool readProperty(PxTriangleMesh*& outProp )
		{
			outProp = readReference<PxTriangleMesh>();
			return true;
		}
		
		inline bool readProperty(PxHeightField*& outProp )
		{
			outProp = readReference<PxHeightField>();
			return true;
		}

		inline bool readProperty( PxRigidActor *& outProp )
		{
			outProp = readReference<PxRigidActor>();
			return true;
		}
