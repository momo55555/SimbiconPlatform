...
namespace physx { namespace repx {
	...
	class CVariableMemoryPool
	{
		...
		FoundationWrapper& mWrapper;
		TPxU8PtrList mAllMemory;
		SVariableMemPoolNode* mFirstFreeNode;
		PxU32 mMinAllocationSize;

	public:
		CVariableMemoryPool(FoundationWrapper& inWrapper, PxU32 inMinAllocationSize = 0x2000 ) 
			: mWrapper( inWrapper )
			, mAllMemory( inWrapper )
			, mFirstFreeNode( NULL ) 
		...
		PxU8* MarkMem( PxU8* inMem, TMemAllocSizeType inSize )
		{
			...
			TMemAllocSizeType* theMem = reinterpret_cast<TMemAllocSizeType*>( inMem );
			theMem[0] = inSize;
		...
		PxU8* allocate( PxU32 size )
		{
			...
			//of the memory block.
            // SD: pad to 64bits, ARM does not like byte aligned ptrs. Would happen when block is split.
			PxU32 theRequestedSize = (size + sizeof(TMemAllocSizeType))+7 & ~7;
			if ( theRequestedSize < sizeof( SVariableMemPoolNode ) )
				theRequestedSize = sizeof( SVariableMemPoolNode );

			if ( mFirstFreeNode )
			{
				SVariableMemPoolNode* theNode = mFirstFreeNode;
				SVariableMemPoolNode* thePreviousNode = NULL;
				while( theNode )
				{
					if ( theRequestedSize < theNode->mSize )
					{
						//Remove the node from the linked list.
						if ( thePreviousNode )
							thePreviousNode->mNextNode = theNode->mNextNode;
						else
							mFirstFreeNode = theNode->NextNode();
						
						PxU8* theMem( reinterpret_cast< PxU8* >( theNode ) );
						TMemAllocSizeType theSize = theNode->mSize;
						//Split the node?  Allow fragmentation?
						//Split node if size allows up to half the node size.
						TMemAllocSizeType halfSize = theSize / 2;
						if ( halfSize > theRequestedSize
							&& halfSize > sizeof(SVariableMemPoolNode) )
						{
							theSize = theRequestedSize;
							PxU8* theLeftover = theMem + theRequestedSize;
							AddFreeMem( theLeftover, theNode->mSize - theRequestedSize );
						}
						//Not splitting the memory, return more than requested
						//and leave it at that.
						return MarkMem( theMem, theSize );
					}
					else
					{
						thePreviousNode = theNode;
						theNode = theNode->NextNode();
						CheckFreeListInvariant( thePreviousNode );
					}
				}
			}
			if ( theRequestedSize < mMinAllocationSize )
			{
				PxU8* newMem = (PxU8*)mWrapper.getAllocator().allocate( (size_t)mMinAllocationSize, "RepX variable sized memory pool", __FILE__, __LINE__ );
				mAllMemory.pushBack( newMem );
				AddFreeMem( newMem, mMinAllocationSize );
		...
		void deallocate( PxU8* inData )
		{
			TMemAllocSizeType* theData = reinterpret_cast< TMemAllocSizeType* >( inData ) - 1;
			TMemAllocSizeType theSize = *(theData);
		...
		void AddFreeMem( PxU8* inMemory, TMemAllocSizeType inSize )
		{
			...
			if ( mFirstFreeNode == NULL )
				mFirstFreeNode = theNewNode;
			else
			{
				SVariableMemPoolNode* theNode = mFirstFreeNode->NextNode();
				SVariableMemPoolNode* thePreviousNode = mFirstFreeNode;
				//Linear search inserting at first available location.
				while( theNode && theNode->mSize < inSize )
				{
					thePreviousNode = theNode;
					theNode = theNode->NextNode();
					CheckFreeListInvariant( thePreviousNode );
				}
				//The new node goes after the previous node
				if ( thePreviousNode->mSize < theNewNode->mSize )
				{
					thePreviousNode->mNextNode = theNewNode;
					theNewNode->mNextNode = theNode;
				}
				else //The first node was bigger than the new node;
					//the new node becomes the first free node.
				{
					theNewNode->mNextNode = mFirstFreeNode;
					mFirstFreeNode = theNewNode;
				}
			}
		}
