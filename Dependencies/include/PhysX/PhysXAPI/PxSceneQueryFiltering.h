...
struct PxSceneQueryFilterFlag
{
	enum Enum
	{
		eSTATIC				= (1<<0),						//!< Traverse static shapes -- WARNING: must be 1<<0
		eDYNAMIC			= (1<<1),						//!< Traverse dynamic shapes -- WARNING: must be 1<<1
		...
		eMESH_MULTIPLE		= (1<<4),						//!< Generate all hits for meshes rather than just the first - NOT CURRENTLY SUPPORTED
...
\li If the data field is non-zero, and the bitwise-AND value of data AND the shape's
queryFilterData is zero, the shape is skipped, else the filter callbacks will be get invoked
according to the specification in the flags field (see #PxSceneQueryFilterFlags). If neither 
#PxSceneQueryFilterFlag::ePREFILTER or #PxSceneQueryFilterFlag::ePOSTFILTER is set, the hit is 
...
The prefilter may overwrite the copy of filterFlags passed in, in order to specify #PxSceneQueryFilterFlag::eBACKFACE and 
...
public:
	...
	\param[in,out] filterFlags The query filter flags (only the flags eMESH_MULTIPLE, eBACKFACE can be modified)
