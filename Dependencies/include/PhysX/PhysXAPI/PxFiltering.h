...
struct PxPairFlag
{
	enum Enum
	{
		...
		*/
		...
		/**
		\brief Provide the (summed total) friction force and normal force in contact reports for this collision pair (see #PxContactPair).

		\note Only takes effect if the colliding actors are rigid bodies and if used in combination with the flags eNOTIFY_TOUCH_... or eNOTIFY_THRESHOLD_FORCE_...

		@see PxSimulationEventCallback.onContact() PxContactPair
		*/
		eNOTIFY_CONTACT_FORCES				= (1<<9) | eRESOLVE_CONTACTS,

		/**
		\brief Provide contact forces per contact point in contact reports for this collision pair.

		\note Only takes effect if the colliding actors are rigid bodies.

		@see PxSimulationEventCallback.onContact() PxConstContactStream PxContactStreamIterator PxContactStreamIterator.getPointNormalForce()
		*/
		eNOTIFY_CONTACT_FORCE_PER_POINT	= (1<<10) | eNOTIFY_CONTACT_POINTS | eRESOLVE_CONTACTS,

		/**
		\brief Provide feature indices per contact point in contact reports for this collision pair.

		\note Only takes effect if the colliding actors are rigid bodies.

		@see PxSimulationEventCallback.onContact() PxConstContactStream PxContactStreamIterator PxContactStreamIterator.getFeatureIndex0()
		*/
		eNOTIFY_CONTACT_FEATURE_INDICES_PER_POINT = (1<<11) | eNOTIFY_CONTACT_POINTS,

		/**
		\brief Enables swept contact generation for this pair.  While more costly, with swept contact generation objects pressed into each other
		will not tunnel through each other.  It is best used for pairs involving important small or thin objects.
		...
		@see eSWEPT_INTEGRATION_LINEAR
		@see eSWEPT_INTEGRATION_FULL
		*/
		eSWEPT_CONTACT_GENERATION			= (1<<12),

		/**
		\brief A less expensive approximation of eSWEPT_INTEGRATION_FULL, where the rotational motion of the objects is neglected.  Should be used when
		performance is of the essence, for objects where angular motion is unlikely to lead to tunneling.
		...
		@see PxSceneFlag::eENABLE_SWEPT_INTEGRATION
		@see eSWEPT_INTEGRATION_FULL
		@see eSWEPT_CONTACT_GENERATION
		*/
		eSWEPT_INTEGRATION_LINEAR			= (1<<13),

		/**
		\brief Enables swept integration for this pair.  Pairs which have this feature enabled check whether the motion of the involved shapes during integration
		intersect, thus preventing shapes from tunneling through each other.  Kinematic motion is taken into account, but movement due to repositioning by the user is not.
		...
		@see eSWEPT_CONTACT_GENERATION
		*/
		eSWEPT_INTEGRATION_FULL				= (1<<14) | eSWEPT_INTEGRATION_LINEAR,
...
struct PxFilterFlag
{
	enum Enum
	{
		...
		\li The user enforces a re-filtering (see #PxShape::resetFiltering(), #PxDeformable::resetFiltering(), #PxParticleSystem::resetFiltering())

		@see PxShape::resetFiltering() PxDeformable::resetFiltering() PxParticleBase::resetFiltering()
		*/
		eKILL				= (1<<0),
...
@see PxShape.setSimulationFilterData() PxShape.getSimulationFilterData() PxDeformableDesc.simulationFilterData PxParticleBaseDesc.simulationFilterData PxSimulationFilterShader PxSimulationFilterCallback
*/
struct PxFilterData
{
...
\li A re-filtering was forced through resetFiltering() (see #PxShape::resetFiltering(), #PxDeformable::resetFiltering(), #PxParticleBase::resetFiltering())
...
\li Pair of deformables
\li A particle system and a deformable
...
public:
	...
	\param[in] s0 Shape pointer of the first object (NULL if the object has no shapes, for example in the case of a deformable or particle system)
	\param[in] attributes1 The filter attribute of the second object
	\param[in] filterData1 The custom filter data of the second object
	\param[in] a1 Actor pointer of the second object
	\param[in] s1 Shape pointer of the second object (NULL if the object has no shapes, for example in the case of a deformable or particle system)
