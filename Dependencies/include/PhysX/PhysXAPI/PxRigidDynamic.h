...
struct PxRigidDynamicFlag
{
	enum Enum
	{
		...
		mass and can be moved around the world using the moveKinematic() method. They will push 
		...
		@see PxRigidDynamic.moveKinematic()
		*/
...
// PX_SERIALIZATION
								...
								PX_DECLARE_SERIAL_RTTI(PxRigidDynamic, PxRigidBody)
//~PX_SERIALIZATION
...
*/
	...
	out (i.e. the motion is not blocked due to joints or collisions) will
	move the body into the desired pose. After the move is carried out during
	...
	*/
	virtual		void				moveKinematic(const PxTransform& destination) = 0;
