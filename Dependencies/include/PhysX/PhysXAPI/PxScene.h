...
#include "geometry/PxDeformableMesh.h"
#endif
...
/** 
 \brief A scene is a collection of bodies, deformables, particle systems and constraints which can interact.

 The scene simulates the behavior of these objects over time. Several scenes may exist 
 at the same time, but each body, deformable, particle system or constraint is specific to a scene 
...
class PxScene
{
	...
	Removes any actors,  particle systems, deformables and constraint shaders from this scene
...
//@{
	...
	*/
	...
	\note If the actor is a PxDeformable then each assigned PxAttachment object will get added to the scene automatically if
	it connects to another actor that is part of the scene already.
	...
	*/
	...
	\note If the actor is a PxDeformable then all assigned PxAttachment objects will get removed from the scene automatically.
...
//@{
	...
	*/
	...
	/**
	\brief Returns the number of attachments in the scene.

	\return the number of attachments in this scene.
	...
	*/
	...
	/**
	\brief Retrieve all the attachments in the scene.

	\param[out] userBuffer The buffer to receive attachment pointers.
	\param[in] bufferSize Size of provided user buffer.
	\param[in] startIndex Index of first attachment pointer to be retrieved
	\return Number of attachments written to the buffer.

	@see getNbAttachments()
	*/
	virtual	PxU32						getAttachments(PxAttachment** userBuffer, PxU32 bufferSize, PxU32 startIndex=0) const = 0;
...
//@{
	...
	*/
	...
	@see PxClientBehaviorBit PxClientID createClient() getClientBehaviorBits()
	*/
...
/************************************************************************************************/
	...
	\param[in] completionTask if non-NULL, this task will have its refcount decremented when the scene is
	ready to have fetchResults called. The refCount is initialized to 2, so that the application must also
	call release on the task for it to run.
	...
	*/
	virtual	void						simulate(PxReal elapsedTime, physx::pxtask::BaseTask *completionTask = NULL, bool controlSimulation = true)		= 0;
	...
	*/
	...
	\param[in] sendPendingReports When set to true pending reports will be sent out before the buffers get cleaned up (for instance lost touch contact reports due to deleted objects).
	*/
	...
	*/
	...
	\note Shooting a ray from within an object leads to different results depended on the shape type. Please check the details in article SceneQuery. User can ignore such objects by using one of the provided filter mechanisms.
	...
	*/
	virtual bool raycastAny(const PxVec3& origin, const PxVec3& unitDir, const PxReal distance,
							...
							PxClientID queryClient = PX_DEFAULT_CLIENT) = 0;
	...
	*/
	...
	\param[in] distance			Length of the ray. Needs to be larger than 0.
	\param[in] outputFlags		Specifies which properties should be written to the hit information
	\param[in,out] hitBuffer	Raycast hit information buffer. If the buffer overflows, the blocking hit is returned as the last entry together with an arbitrary subset
	...
	*/
	...
	\param[in] geometry		Geometry of object to sweep (supported types are: box, sphere, capsule).
	...
	\param[in] queryFlags	Lets the user filter out initially overlapping objects, if necessary
	...
	*/
	...
	\param[in] geometryList		Geometries of objects to sweep (supported types are: box, sphere, capsule).
	...
	*/
	...
	\param[in] distance			Sweep distance. Needs to be larger than 0. Will be clamped to PX_MAX_SWEEP_DISTANCE.
	\param[in] outputFlags		Specifies which properties should be written to the hit information.
	\param[in,out] hitBuffer	Sweep hit information buffer. If the buffer overflows, the blocking hit is returned as the last entry together with an arbitrary subset
								of the nearer touching hits (typically the query should be restarted with a larger buffer).
	\param[in] hitBufferSize	Size of the hit buffer.
	\param[out] blockingHit		True if a blocking hit was found. If found, it is the last in the buffer, preceded by any touching hits which are closer. Otherwise the touching hits are listed.
	\param[in] filterData		Filtering data and simple logic.
	...
	*/
	...
	\param[in] geometry			Geometry of object to check for overlap (supported types are: box, sphere, capsule).
	\param[in] pose				Pose of the object.
	\param[in,out] hit			Pointer to store the overlapping object to.
	\param[in] filterData		Filtering data and simple logic.
	...
	*/
	...
	\param[in] geometry			Geometry of object to check for overlap (supported types are: box, sphere, capsule).
	\param[in] pose				Pose of the object.
	\param[in,out] hitBuffer	Buffer to store the overlapping objects to. If the buffer overflows, an arbitrary subset of overlapping objects is stored (typically the query should be restarted with a larger buffer).
	\param[in] hitBufferSize	Size of the hit buffer.
	\param[in] filterData		Filtering data and simple logic.
	\param[in] filterCall		Custom filtering logic (optional). Only used if the corresponding #PxSceneQueryFilterFlag flags are set. If NULL, all hits are assumed to overlap.
	\param[in] queryClient		ID of the client doing the query (see #createClient())
