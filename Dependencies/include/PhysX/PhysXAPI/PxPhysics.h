...
//~PX_AGGREGATE
	...
	*/
	PX_DEPRECATED virtual PxDeformable*		createDeformable(const PxDeformableDesc& deformableDesc)		= 0;
	...
	@see PxDeformableDesc PxDeformable
	*/
	PX_DEPRECATED virtual PxAttachment*       createAttachment(PxDeformable& deformable , PxShape* shape, PxU32 nbVertices, const PxU32* vertexIndices, const PxVec3* positions, const PxU32* flags) = 0;
#endif
	...
	*/
	PX_DEPRECATED virtual PxDeformableMesh*	createDeformableMesh(const PxStream& stream) = 0;
	...
	*/
	PX_DEPRECATED virtual PxU32				getNbDeformableMeshes() const = 0;
	...
	\param[in] startIndex Index of first deformable mesh pointer to be retrieved
	...
	*/
	PX_DEPRECATED virtual	PxU32				getDeformableMeshes(PxDeformableMesh** userBuffer, PxU32 bufferSize, PxU32 startIndex=0) const = 0;
#endif
	...
	@see PxClothFabric PxClothFabric.release() PxStream PxCloth
	*/
	virtual PxClothFabric*		createClothFabric(const PxStream& stream) = 0;
	...
	\note We recommended to use #PxCooking.cookClothFabric() to create cloth fabrics from meshes and then use createClothFabric(const PxStream& stream).
	...
	\param[in] phases array with an index per phase which points one entry beyond the last fiber of the phase. See #PxClothFabric.getPhases().
	\param[in] phaseTypes array defining the type of each phase. See #PxClothFabricPhaseType.
	\param[in] nbPhases number of phases in the fabric.
	...
	\param[in] nbFibers number of fibers in the fabric.
	\param[in] particleIndices array of particle indices which specifies the line strips of connected particles of a fiber. See #PxClothFabric.getParticleIndices().
	\param[in] nbParticleIndices number of particle indices.
	\param[in] restlengths array of rest length values for each connected particle pair. See #PxClothFabric.getRestlengths().
	\return The new cloth fabric.

	@see PxClothFabric PxClothFabric.release() PxCloth
	*/
	virtual PxClothFabric*		createClothFabric(PxU32 nbParticles, const PxU32* phases, const PxClothFabricPhaseType::Enum* phaseTypes, PxU32 nbPhases, const PxU32* fibers, PxU32 nbFibers, 
												const PxU32* particleIndices, PxU32 nbParticleIndices, const PxReal* restlengths) = 0;
...
*/
PX_C_EXPORT PX_PHYSX_CORE_API physx::pubfnd3::PxU32 PX_CALL_CONV PxGetValue(physx::PxCookingValue::Enum cookValue);
...
*/
PX_C_EXPORT PX_PHYSX_CORE_API physx::PxPhysics* PX_CALL_CONV PxCreatePhysics(physx::pubfnd3::PxU32 version,
...
*/
PX_C_EXPORT PX_PHYSX_CORE_API physx::PxFoundation* PX_CALL_CONV PxGetFoundation();
