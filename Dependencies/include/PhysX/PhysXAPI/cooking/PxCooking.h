...
struct PxCookingParams
{
	...
	*/
	...
	{
#if defined(PX_X86) || defined(PX_X64) || defined(PX_LINUX)
...
public:
	...
	*/
	virtual bool  cookTriangleMesh(const PxTriangleMeshDesc& desc, PxStream& stream) = 0;
	...
	*/
	virtual bool  cookConvexMesh(const PxConvexMeshDesc& desc, PxStream& stream)= 0;

	/**
	\brief Cooks a triangle / tetrahedron mesh to a PxDeformableMesh.

	\param desc The deformable mesh descriptor on which the generation of the cooked mesh depends.
	\param stream The stream the cooked mesh is written to.
	\return True if cooking was successful
	*/
	virtual bool  cookDeformableMesh(const PxDeformableMeshDesc& desc, PxStream& stream) = 0;
	...
	\return True if cooking was successful
	*/
	virtual bool  cookClothFabric(const PxClothMeshDesc& desc, const PxVec3& gravityDir, PxStream& stream) = 0;
...
PX_C_EXPORT PX_PHYSX_COOKING_API physx::PxCooking* PX_CALL_CONV PxCreateCooking(physx::pubfnd3::PxU32 version,
