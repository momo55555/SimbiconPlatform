...
public:
	...
	PxU32									numSpheres;  //!< total number of spheres
	const PxClothCollisionSphere*			spheres; //!< sphere data array
	PxU32									numPairs; //!< number of capsules
...
{
	if(numSpheres && (!spheres)) 	// missing info for specified number of spheres
		return false;
	if(numPairs && !numSpheres)  // can only specify pairs, if there are spheres
		return false;
	if (numPairs && !pairIndexBuffer)  // missing index info for specified number of pairs
		return false;
