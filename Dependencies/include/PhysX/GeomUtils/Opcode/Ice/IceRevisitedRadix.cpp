...
#if defined(__BIG_ENDIAN__) || defined(_XBOX) || defined(__CELLOS_LV2__) || defined(PX_WII) 
...
#define CREATE_HISTOGRAMS(type, buffer)														\
	...
	Ps::memZero(mHistogram1024, 256*4*sizeof(PxU32));										\
																							\
	/* Prepare to count */																	\
	PxU8* PX_RESTRICT p = (PxU8* PX_RESTRICT)input;											\
	PxU8* PX_RESTRICT pe = &p[nb*4];														\
	...
	{																						\
		...
		type* PX_RESTRICT Running = (type* PX_RESTRICT)buffer;								\
...
//		CHECK_PASS_VALIDITY(j);
		...
		if(CurCount)
		{
			...
			// Perform Radix Sort
			PxU8* PX_RESTRICT InputBytes	= (PxU8* PX_RESTRICT)input;
...
RadixSort& RadixSort::Sort(const float* input2, PxU32 nb)
{
	...
	PxU32* PX_RESTRICT input = (PxU32* PX_RESTRICT)input2;
