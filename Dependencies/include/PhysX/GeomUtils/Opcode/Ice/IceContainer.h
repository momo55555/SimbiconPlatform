...
namespace Ice
{
	...
	class Container : public Ps::UserAllocated
	{
		public:
										Container();
		...
		PX_FORCE_INLINE	Container&		Add(const PxU32* entries, PxU32 nb)
										{
											if(entries && nb)
											{
												...
												// Add new entry
												Ps::memCopy(&mEntries[mCurNbEntries], entries, nb*sizeof(PxU32));
												mCurNbEntries += nb;
											}
											return *this;
										}
		...
		PX_FORCE_INLINE	PxU32*			Reserve(PxU32 nb)
										{
											...
											// PT: leap of faith...
											Ps::invalidateCache(Reserved, nb*sizeof(PxU32));
		...
		PX_FORCE_INLINE	Container&		Add(const float* entries, PxU32 nb)
										{
											...
											// Add new entry
											Ps::memCopy(&mEntries[mCurNbEntries], entries, nb*sizeof(float));
											mCurNbEntries+=nb;
											return *this;
										}
		...
		PX_FORCE_INLINE	Ps::IntBool		IsNotEmpty()					const	{ return mCurNbEntries;					}	//!< Checks the container is empty
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    class ContainerSizeT : public Ps::UserAllocated
	{
		public:
										ContainerSizeT();
										~ContainerSizeT();
		 ...
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
						void			InitSharedBuffers(PxU32 max_entries, size_t* entries);
		 ...
		 *	costs a lot more than the call overhead...
		 *
		 *	\param		entry		[in] a size_t to store in the container
		 *	\see		Add(float entry)
		 *	\see		Empty()
		 *	\see		Contains(size_t entry)
		 *	\return		Self-Reference
		 */
		///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		PX_FORCE_INLINE	ContainerSizeT&	Add(size_t entry)
										{
											// Resize if needed
											if(mCurNbEntries==mMaxNbEntries)	Resize();
		...
		//! Add unique [slow]
		PX_FORCE_INLINE	ContainerSizeT&	AddUnique(size_t entry)
										{
											if(!Contains(entry))	Add(entry);
											return *this;
										}								
		...
		// Checks whether the container already contains a given value.
						bool			Contains(size_t entry, size_t* location=NULL) const;
		// Deletes an entry - doesn't preserve insertion order.
						bool			Delete(size_t entry);
		//! Deletes the entry whose index is given
		PX_FORCE_INLINE	void			DeleteIndex(PxU32 index)				{ mEntries[index] = mEntries[--mCurNbEntries];	}

						PxU32			DeleteKeepingOrder(size_t entry);

		// Data access.
		PX_FORCE_INLINE	PxU32			GetNbEntries()					const	{ return mCurNbEntries;					}	//!< Returns the current number of entries.
		PX_FORCE_INLINE	PxU32			GetMaxNbEntries()				const	{ return mMaxNbEntries;					}	//!< Returns maximum number of entries before resizing.
		PX_FORCE_INLINE	size_t			GetEntry(PxU32 i)				const	{ return mEntries[i];					}	//!< Returns ith entry.
		PX_FORCE_INLINE	size_t*			GetEntries()					const	{ return mEntries;						}	//!< Returns the list of entries.
		...
		// Data
						PxU32			mMaxNbEntries;		//!< Maximum possible number of entries
						PxU32			mCurNbEntries;		//!< Current number of entries
						size_t*			mEntries;			//!< List of entries
						float			mGrowthFactor;		//!< Resize: new number of entries = old number * mGrowthFactor
	};
