...
/*	PX_INLINE void GetEdgeIndices(SharedEdgeIndex edge_index, PxU32& id0, PxU32& id1)
	...
	struct CommonWalkParams
	{
		...
		PxU32			SubmeshIndex;	//!< Current submesh index
		void*			UserData;		//!< User-provided data
	};

	typedef void (*WALK_CALLBACK) (const CommonWalkParams& cwp , PxU32 cur_face, PxU32 prev_face, PxU32 crossed_edge, PxU32 prev_edge);
...
#ifdef MSH_ADJACENCIES_INCLUDE_CONVEX_BITS
		PX_INLINE	Ps::IntBool		HasActiveEdge01()								const	{ return IS_CONVEX_EDGE(mATri[EDGE01]);		}
		PX_INLINE	Ps::IntBool		HasActiveEdge20()								const	{ return IS_CONVEX_EDGE(mATri[EDGE02]);		}
		PX_INLINE	Ps::IntBool		HasActiveEdge12()								const	{ return IS_CONVEX_EDGE(mATri[EDGE12]);		}
		PX_INLINE	Ps::IntBool		HasActiveEdge(PxU32 i)							const	{ return IS_CONVEX_EDGE(mATri[i]);			}
#endif
//		private:
...
//									Epsilon	= 0.001f;
	...
	class Adjacencies : public Ps::UserAllocated
	{
		...
		// PT: completely experimental version to improve HW mesh cooker
				bool				Walk2(WALK_CALLBACK callback, void* user_data, const PxVec3* verts, const Gu::TriangleT<PxU32>* tris);
		// Helpers
				PxU32				ComputeNbBoundaryEdges() const;
#ifdef MSH_ADJACENCIES_INCLUDE_TOPOLOGY
