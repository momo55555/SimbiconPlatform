							...
							Ps::aos::FloatV& tnear, Ps::aos::FloatV& tfar)
{
	...
	const FloatV eps = FloatV_From_F32(LOCAL_EPSILON);
	...
	const Vec3V v0 = Vec4V_From_F32Array_Aligned(tmp);
	const Vec3V v1 = Vec4V_From_F32Array_Aligned(tmp1);
	...
	const BoolV con0 = BAllTrue3(V3IsGrtrOrEq(x, tSlab0));
	const BoolV con1 = BAllTrue3(V3IsGrtrOrEq(y, tSlab0));
	...
	return (int)PxF32_From_FloatV(ret);
	...
	const PxVec3& minimum, const PxVec3& maximum, const PxVec3& ro, const PxVec3& rd, float maxDist, float& tnear, float& tfar)
{
	...
	PxF32 invDx = i::recipFast(i::selectMax(i::abs(rd.x), 1e-9f) * i::sign(rd.x));
	...
	PxF32 invDy = i::recipFast(i::selectMax(i::abs(rd.y), 1e-9f) * i::sign(rd.y));
	...
	PxF32 invDz = i::recipFast(i::selectMax(i::abs(rd.z), 1e-9f) * i::sign(rd.z));
					   ...
					   Ps::aos::FloatV& tnear, Ps::aos::FloatV& tfar)
{
	...
	const Vec3V eps = FloatV_From_F32(1e-9f);
