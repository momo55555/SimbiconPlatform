...
namespace Gu
{
	...
	const PxF32*	getBoxVertexNormals();
	const PxU8*		getBoxTriangles();
	const PxVec3*	getBoxLocalEdgeNormals();
	const PxU8*		getBoxEdges();

	void			computeBoxPoints(const PxBounds3& bounds, PxVec3* PX_RESTRICT pts);

	void			computeBoxWorldEdgeNormal(const Box& box, PxU32 edge_index, PxVec3& world_normal);
	void			computeBoxAroundCapsule(const Capsule& capsule, Box& box);  //TODO: Refactor this one out in the future
	void			computeBoxAroundCapsule(const CapsuleV& capsule, BoxV& box);  //TODO: Refactor this one out in the future
	void			computeBoxAroundCapsule(const PxCapsuleGeometry& capsuleGeom, const PxTransform& capsulePose, Box& box);

	void			getWorldSegment(Gu::Segment& worldSegment, const PxCapsuleGeometry& capsuleGeom, const PxTransform& pose);
	void			getWorldSegment(Gu::SegmentV& worldSegment, const PxCapsuleGeometry& capsuleGeom, const PxTransform& pose);
	void			getWorldCapsule(Gu::Capsule& worldCapsule, const PxCapsuleGeometry& capsuleGeom, const PxTransform& pose);
	Gu::Plane		getPlane(const PxTransform& pose);
	Gu::PlaneV      getPlaneV(const PxTransform& pose);
	PxTransform		getWorldTransform(const Gu::Capsule& worldCapsule, PxReal& halfHeight);
