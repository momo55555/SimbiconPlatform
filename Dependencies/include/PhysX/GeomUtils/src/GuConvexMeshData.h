...
namespace Gu
{
	...
	struct HullPolygonData
	{
		...
		Plane			mPlane;			//!< Plane equation for this polygon	//Could drop 4th elem as it can be computed from any vertex as: d = - p.dot(n);
		...
		{ 
			return mPlane.normal.dot(hullVertices[mMinIndex]);
		}
