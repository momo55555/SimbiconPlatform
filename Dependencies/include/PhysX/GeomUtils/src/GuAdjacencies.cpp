 ...
 *
 *	So that ones ARE useful.... The code could be modified in order to support plain pointers, which would be even faster.
 *
 *	1.0:
 *	- initial version
 *
 *	1.1:
 *	- SharedEdgeIndex enum added.
 *	- CreateDatabase() now private, automatically called by Init().
 *
 *	1.2:
 *	- user data added in walking callback
 *	- temporary edges & internal building methods removed from the interface
 *	- topology removed. It was redundant since the topology is available as source data anyway.
 *
 *	1.3:
 *	- support for convex bits added
 *
 *	1.3b:
 *	- fixed a bug introduced in 1.3 (invalid IS_BOUNDARY macro)
 *
 *	\class		Adjacencies
 *	\author		Pierre Terdiman
 *	\version	1.3b
...
#endif
{
	...
	// Init
	Ps::memZero(bound_status, nb_verts*sizeof(bool));
...
/**
 *	Walks the surface.
 *	\param		callback	[in] the walk callback
 *	\param		user_data	[in] user-data sent to callback
 *	\return		true if success
 */
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool Adjacencies::Walk(WALK_CALLBACK callback, void* user_data)
{
	struct PrivateWalkParams : CommonWalkParams
	{
		bool*			Markers;		// Face markers
		WALK_CALLBACK	Callback;		// App callback
	};

	struct Local
	{
		// Recursive local walking routine
		static void _WalkFace(PrivateWalkParams& cwp, PxU32 cur_face, PxU32 prev_face, PxU32 crossed_edge, PxU32 prev_edge)
		{
			if(cwp.Markers[cur_face])	return;	// Already processed

			// Get current triangle
			AdjTriangle* CurTri = &cwp.Adj->mFaces[cur_face];

			// Call the app
			(cwp.Callback)(cwp, cur_face, prev_face, crossed_edge, prev_edge);

			// Jump to adjacent faces
			PxU32 Link01 = CurTri->mATri[0];
			PxU32 Link02 = CurTri->mATri[1];
			PxU32 Link12 = CurTri->mATri[2];

			PxU32 Id01 = MAKE_ADJ_TRI(Link01);		PxU32 Edge01 = GET_EDGE_NB(Link01);
			PxU32 Id02 = MAKE_ADJ_TRI(Link02);		PxU32 Edge02 = GET_EDGE_NB(Link02);
			PxU32 Id12 = MAKE_ADJ_TRI(Link12);		PxU32 Edge12 = GET_EDGE_NB(Link12);

			cwp.Markers[cur_face] = true;

			// Now jump to adjacent faces
			if(!IS_BOUNDARY(Link01))	_WalkFace(cwp, Id01, cur_face, Edge01, 0);
			if(!IS_BOUNDARY(Link02))	_WalkFace(cwp, Id02, cur_face, Edge02, 1);
			if(!IS_BOUNDARY(Link12))	_WalkFace(cwp, Id12, cur_face, Edge12, 2);
		}
	};

	// Checkings
	if(!mFaces || !mNbFaces)	return false;

	// Create markers to tag processed faces
	bool* Markers = (bool*)PX_ALLOC_TEMP(sizeof(bool)*mNbFaces);
	Ps::memZero(Markers, mNbFaces*sizeof(bool));
	...
	do
	{
		// Look for a non-processed face
		i = 0;
		while(i<mNbFaces && Markers[i])	i++;

		// Recurse from current face
		if(i!=mNbFaces)
		{
			PrivateWalkParams CWP;
			CWP.Adj				= this;
			CWP.Markers			= Markers;
			CWP.SubmeshIndex	= NbSubmeshes;
			CWP.Callback		= callback;
			CWP.UserData		= user_data;
			Local::_WalkFace(CWP, i, PX_INVALID_U32, PX_INVALID_U32, PX_INVALID_U32);
		}
		NbSubmeshes++;
	...
	// Free local ram
	PX_FREE_AND_RESET(Markers);

	return true;
}

bool Adjacencies::Walk2(WALK_CALLBACK callback, void* user_data, const PxVec3* verts, const Gu::TriangleT<PxU32>* tris)
{
	struct PrivateWalkParams : CommonWalkParams
	{
		bool*						Markers;		// Face markers
	...
	struct Local
	{
		...
		static void _WalkFace(PrivateWalkParams& cwp, PxU32 cur_face, PxU32 prev_face, PxU32 crossed_edge, PxU32 prev_edge)
		{
			...
			// Now jump to adjacent faces
			PxU32 Links[]= { Link01, Link02, Link12 };
			PxU32 Indices[]= { Id01, Id02, Id12 };
			PxU32 Edges[]	= { Edge01, Edge02, Edge12 };
			PxU32 Crossed[] = { 0, 1, 2 };
			{
				float d[] = { PX_MAX_F32, PX_MAX_F32, PX_MAX_F32 };
				if(!IS_BOUNDARY(Link01))	{ PxVec3 tmp;	cwp.Tris[Id01].center(cwp.Verts, tmp);	d[0] = (cwp.Origin - tmp).magnitudeSquared();	}
				if(!IS_BOUNDARY(Link02))	{ PxVec3 tmp;	cwp.Tris[Id02].center(cwp.Verts, tmp);	d[1] = (cwp.Origin - tmp).magnitudeSquared();	}
				if(!IS_BOUNDARY(Link12))	{ PxVec3 tmp;	cwp.Tris[Id12].center(cwp.Verts, tmp);	d[2] = (cwp.Origin - tmp).magnitudeSquared();	}
				...
				for(PxU32 i=0;i<3;i++)
				{
					for(PxU32 j=0;j<2;j++)
					{
						...
						{
							TSwap(d[j], d[j+1]);
							TSwap(Links[j], Links[j+1]);
							TSwap(Indices[j], Indices[j+1]);
							TSwap(Edges[j], Edges[j+1]);
							TSwap(Crossed[j], Crossed[j+1]);
						}
					}
				}
			}
			for(PxU32 i=0;i<3;i++)
			{
				if(!IS_BOUNDARY(Links[i]))	_WalkFace(cwp, Indices[i], cur_face, Edges[i], Crossed[i]);
			}
...
/*			if(!IS_BOUNDARY(Link01))	_WalkFace(cwp, Id01, cur_face, Edge01, 0);
	...
	do
	{
		...
		if(i!=mNbFaces)
		{
			...
			CWP.Tris			= tris;
			CWP.Count			= 0;
			tris[i].center(verts, CWP.Origin);	// Setup origin for this chunk
			Local::_WalkFace(CWP, i, PX_INVALID_U32, PX_INVALID_U32, PX_INVALID_U32);
		}
		NbSubmeshes++;
