...
namespace Gu
{
	...
	public:
		...
		{
			isFace = 0;
			return pointOnSurface.getNormalized();
		}
...
/*			const PxVec3 p0(mHalfHeight, 0.0f, 0.0f);
		...
		virtual PxVec3 inverseSupportMapping(const PxVec3& pointOnSurface, int& isFace) const
		{
			...
			return (pointOnSurface - pClosest).getNormalized();
		}
	...
	public:
		...
		PxVec3 inverseSupportMapping(const PxVec3& pointOnSurface, int& isFace) const
		{
			...
			PxF32 cntzEps = 1e-2f;
			PxU32 cntz = (absP.x < cntzEps) + (absP.y < cntzEps) + (absP.z < cntzEps);
			isFace = (cntz == 1);
	...
	{
	public:
		GJKConvexSupport(const Gu::ConvexMesh& cm, const PxMeshScale& scale) : mConvexMesh(cm), mVertex2ShapeSkew(scale.toMat33())
		{
		}
		virtual void getBounds(PxBounds3& bounds) const {}		//not needed
		virtual PxVec3 projectHullMax(const PxVec3& localDir, GJKConvexInterfaceCache&) const
		{
			PxReal min, max;
			return projectHull_(mConvexMesh.getHull(), min, max, localDir, mVertex2ShapeSkew);
		}
		virtual PxVec3 inverseSupportMapping(const PxVec3& pointOnSurface, int& isFace) const
		{
			return hullInverseSupportMapping(mConvexMesh.getHull(), pointOnSurface, isFace, mVertex2ShapeSkew);
		}
	private:
		const Gu::ConvexMesh&	mConvexMesh;
	...
	class GJKTriangleSupport : public GJKConvexInterface
	{
		...
		virtual PxVec3	projectHullMax(const PxVec3& localDir, GJKConvexInterfaceCache&) const
		{
			PxVec3 localDirN = localDir.getNormalized();
			PxReal maximum = -FLT_MAX;
			PxU32 candidate;
			for(PxU32 i=0;i<3;i++)
			{
				const PxReal dp = localDirN.dot(mP[i]);
				if(dp>maximum)
				{
					maximum = dp;
					candidate = i;
		...
		{
			isFace = 1;
			return (mP[1]-mP[0]).cross(mP[2]-mP[0]);
		}
