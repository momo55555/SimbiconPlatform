...
PX_FORCE_INLINE PxMat34Legacy buildMatrixFromBox(const Gu::Box& box)
{
	return PxMat34Legacy(box.getRotLegacy(), box.center);
}
...
PX_INLINE Gu::Box transform(const PxMat34Legacy& transfo, const Gu::Box& box)
{
	Gu::Box ret;
	PxMat33& obbBasis = ret.rot;

	obbBasis.column0 = transfo.M * (box.rot.column0 * box.extents.x);
	obbBasis.column1 = transfo.M * (box.rot.column1 * box.extents.y);
	obbBasis.column2 = transfo.M * (box.rot.column2 * box.extents.z);
