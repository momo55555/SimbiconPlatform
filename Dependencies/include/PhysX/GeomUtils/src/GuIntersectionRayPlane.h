...
namespace Gu
{
	...
	PX_INLINE bool intersectRayPlane(const PxVec3& orig, const PxVec3& dir, const Gu::Plane& plane, float& distanceAlongLine, PxVec3* pointOnPlane = NULL)
	{
		const float dn = dir.dot(plane.normal);
	...
	PX_INLINE Ps::aos::BoolV intersectRayPlane(const Ps::aos::Vec3VArg orig, const Ps::aos::Vec3VArg dir, const Gu::PlaneV& plane, Ps::aos::FloatV& distanceAlongLine, Ps::aos::Vec3V& pointOnPlane)
	{
		...
		const FloatV eps = FloatV_From_F32(1E-7);
