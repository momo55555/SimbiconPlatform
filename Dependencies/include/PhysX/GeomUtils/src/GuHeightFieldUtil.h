...
namespace Gu
{
	...
	private:
		...
		{
			//	mOneOverHeightScale	= (mHfGeom.heightScale > PX_EPS_REAL) ?			PxReal(1) / mHfGeom.heightScale : PX_MAX_REAL;
			//	mOneOverRowScale    = (PxAbs(mHfGeom.rowScale) > PX_EPS_REAL) ?		PxReal(1) / mHfGeom.rowScale    : PX_MAX_REAL;
			//	mOneOverColumnScale = (PxAbs(mHfGeom.columnScale) > PX_EPS_REAL) ?	PxReal(1) / mHfGeom.columnScale : PX_MAX_REAL;
			const PxReal absRowScale = PxAbs(mHfGeom->rowScale);
			const PxReal absColScale = PxAbs(mHfGeom->columnScale);
			mOneOverHeightScale	= physx::intrinsics::fsel(PX_EPS_REAL - mHfGeom->heightScale,	PX_MAX_REAL, PxReal(1) / mHfGeom->heightScale);
			mOneOverRowScale	= physx::intrinsics::fsel(PX_EPS_REAL - absRowScale,			PX_MAX_REAL, PxReal(1) / mHfGeom->rowScale);
			mOneOverColumnScale	= physx::intrinsics::fsel(PX_EPS_REAL - absColScale,			PX_MAX_REAL, PxReal(1) / mHfGeom->columnScale);	
		}
...
//		PxVec3	getNormalAtShapePoint(PxReal x, PxReal z) const;
		...
		PxU32	getFeatureIndexAtShapePoint(PxReal x, PxReal z) const;
		PxU32	getFeatureIndexAtShapePointNoTest(PxReal x, PxReal z) const;
		PxU32	getFeatureIndexAtShapePointNoTest2(PxU32 cell, PxReal fracX, PxReal fracZ) const;
		PxU32	getFeatureIndexAtTriangleIndex(PxU32 triangleIndex) const;
...
//		PxU32	getVertexFeatureIndex(PxU32 vertexIndex) const;
		PxU32	getVertexFeatureIndex(PxU32 vertexIndex, PxU32 row, PxU32 column) const;
		...
		PxU32	getEdgeFeatureIndex(PxU32 edgeIndex, PxU32 cell, PxU32 row, PxU32 column) const;
		PxU32	getEdgeFeatureIndex(PxU32 edgeIndex, PxU32 count, const PxU32* PX_RESTRICT faceIndices) const;

		PxU32	findClosestPointsOnCell(PxU32 row, PxU32 column, PxVec3 point, PxVec3* PX_RESTRICT closestPoints, PxU32* PX_RESTRICT features, bool testEdges = true) const;
...
//		PxReal	findClosestPointOnEdge(PxU32 edgeIndex, const PxVec3& point, PxVec3& closestPoint) const;
		...
		PxU32	getTriangle(const PxTransform&, Gu::Triangle& worldTri, Gu::Triangle* edgeTri, PxU32* returnedFlags, PxU32* vertexIndices, PxTriangleID triangleIndex, bool worldSpaceTranslation=true, bool worldSpaceRotation=true) const;
...
/*			return ((!(x < 0))
		...
		// maxT specifies how far along the line we should accept the results (aP0 + t(aP1-aP0))

		template<class T, bool useUnderFaceCallback, bool backfaceCull>
		PX_INLINE void traceSegment(const PxVec3& aP0, const PxVec3& aP1, T* aCallback, PxF32 maxT = 1.0f) const
		{
			...
			PxReal tnear;
			PxReal tfar;
			if (!Gu::intersectRayAABB2(localBounds.minimum, localBounds.maximum, aP0, aP1-aP0, maxT, tnear, tfar)) 
				return;
			...
			// we don't want to normalize this, so the rate of advance of du, dh, dv stays in sync for triangle intersection
			...
			PxI32 ui = PxI32(du > 0.0f ? PxFloor(u0) : PxCeil(u0));
			PxI32 vi = PxI32(dv > 0.0f ? PxFloor(v0) : PxCeil(v0));
			...
			#endif
			{
				...
				// AP scaffold: clamp temporarily until i can repro this bug
				if(ui+step_ui < 0 || ui+step_ui>=numRows || vi+step_vi < 0 || vi+step_vi>=numCols)
					break;
				...
				#endif
				{
					...
					const PxU32 cellIndex = colIndex0;
					bool isZVS = hf.isZerothVertexShared(cellIndex);
					if (!isZVS)
					{
					...
					{
						const PxF32 tEpsilon = 1e-6f;
						if (Gu::intersectLineTriangle(uhv0, duhv, *p10a, *p00a, *p11a, triT, triU, triV, backfaceCull, enlargeEpsilon) &&
							triT >= -tEpsilon && triT <= 1.0f+tEpsilon)
						{
							PxVec3 hitPoint((u0 + du*triT) * rowScale, h0 + dh * triT, (v0 + dv*triT) * columnScale);
							if (!aCallback->faceHit(*this, hitPoint, cellIndex*2))
								return;
						} else // since we use backface culling we don't have to test the adjacent triangle if we got a hit on the first
						if (Gu::intersectLineTriangle(uhv0, duhv, *p01a, *p11a, *p00a, triT, triU, triV, backfaceCull, enlargeEpsilon)
							&& triT >= -tEpsilon && triT <= 1.0f+tEpsilon)
						{
							PxVec3 hitPoint((u0 + du*triT) * rowScale, h0 + dh * triT, (v0 + dv*triT) * columnScale);
					...
					else
					{
						...
						if (!aCallback->underFaceHit(*this, triNormals[dotPrevGtz], crossedEdge,
								uprev * rowScale, vprev * columnScale, COMPUTE_H_FROM_T(tPrev), triIndex0))
							return;

						if (triIndex1 != triIndex0) // if triIndex0 != triIndex1 that means we cross the triangle edge
						{
