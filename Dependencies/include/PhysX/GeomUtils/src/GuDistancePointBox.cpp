...
Ps::aos::FloatV Gu::distancePointBoxSquared(	const Ps::aos::Vec3VArg point,
												const Ps::aos::Vec3VArg boxOrigin, 
												const Ps::aos::Vec3VArg boxExtent, 
												const Ps::aos::Mat33V& boxBase, 
												Ps::aos::Vec3V& localClosestP)
{
	using namespace Ps::aos;
	const Vec3V nBoxExtent = V3Neg(boxExtent);
	//Translate point into the local space of box
	const Vec3V d = V3Sub(point, boxOrigin);
	const Vec3V b = M33TrnspsMulV3(boxBase, d);
	const Vec3V cb = V3Clamp(b, nBoxExtent, boxExtent);
	const Vec3V v = V3Sub(cb, b);
	localClosestP = cb;
	return V3Dot(v, v);
}
