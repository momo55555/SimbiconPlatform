	...
	/*	PX_INLINE SphereV(const Sphere& sphere)
		...
		*/
		PX_INLINE bool isValid() const
		{
			...
			using namespace Ps::aos;
			return BAllEq(FIsGrtrOrEq(radius, FZero()), BTTTT()) == 1; 
		}
		...
		*/
		PX_INLINE bool contains(const Ps::aos::Vec3VArg minimum, const Ps::aos::Vec3VArg maximum) const
		{
			...
			const Vec3V boxSphereCenter = V3Mul(V3Add(maximum, minimum), half);
			const Vec3V v = V3Mul(V3Sub(maximum, minimum), half);
			...
			return FAllGrtrOrEq(dif, zero) == 1;
		}           
