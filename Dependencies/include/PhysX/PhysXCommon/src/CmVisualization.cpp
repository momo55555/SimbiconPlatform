							  ...
							  const PxTransform& child)
{
	out << parent << Cm::DebugBasis(PxVec3(scale, scale, scale) * 1.5f,
		PxDebugColor::eARGB_DARKRED, PxDebugColor::eARGB_DARKGREEN, PxDebugColor::eARGB_DARKBLUE);
							  ...
							  bool active)
{
	...
	PxTransform r(t0.p+value*t0.q.getBasisVector0(), t0.q*PxQuat(PxPi/2,PxVec3(0,1,0)));
	out << (active ? PxDebugColor::eARGB_RED : PxDebugColor::eARGB_GREY);
	out << PxTransform::createIdentity();
							...
							bool active)
{
	...
	for(PxU32 i=0;i<=32;i++)
	{
		...
		PxVec3 a = q.rotate(PxVec3(1,0,0)) * scale;
							 ...
							 bool active)
{
	...
	PxReal height = sin(angle), radius = cos(angle);

	const int LINES = 32;

	out << Cm::RenderOutput::LINESTRIP;

	PxReal step = PxPi*2/LINES;
