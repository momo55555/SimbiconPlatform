...
namespace Cm
{
	...
	*/
	...
	public:
		...
		{
#if defined(__SNC__)	
			// not sure how to make this a compile time assert with SNC
			// as __COUNTER__ doesn't seem to be getting incremented properly
			PX_ASSERT(EltsPerSlab && !(EltsPerSlab & (EltsPerSlab-1)));
#else
	...
	private:
		...
		bool extend()
		{
			T* mAddr = reinterpret_cast<T*>(PX_ALLOC(EltsPerSlab * sizeof(T)));
			if(!mAddr)
				return false;
