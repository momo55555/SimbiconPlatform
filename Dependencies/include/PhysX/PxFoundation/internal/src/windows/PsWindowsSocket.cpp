...
bool SocketImpl::connect(const char* host, PxU16 port, PxU32 timeout)
{
	...
	memCopy(&socketAddress.sin_addr, hp->h_addr_list[0], hp->h_length);
		...
		if ( mListenSocket != INVALID_SOCKET )
		{
			...
			if(bind(mListenSocket, (sockaddr*)&addr, sizeof(addr)) == 0)
			{
				if(::listen(mListenSocket, SOMAXCONN) == 0)
				{
					...
					Thread::start(Thread::DEFAULT_STACK_SIZE); // start the listener thread.
					ret = true;
				}
			}
		}
...
PxU32 BufferedSocketImpl::write(const PxU8* data, PxU32 length)
{
	...
	while(length > (Socket::DEFAULT_BUFFER_SIZE - mBufferPos))
	{
		...
		memCopy(mBuffer+mBufferPos, data+bytesWritten, currentChunk);
	...
	if(length > 0) 
	{
		memCopy(mBuffer+mBufferPos, data+bytesWritten, length);
		bytesWritten += length;
		mBufferPos += length;
	}
...
Socket::Socket( bool inIsBuffering )
{
	if ( inIsBuffering )
	{
		void* mem = PX_ALLOC(sizeof(BufferedSocketImpl));
		mImpl = PX_PLACEMENT_NEW(mem, BufferedSocketImpl)();
	}
	else
	{
		void* mem = PX_ALLOC(sizeof(SocketImpl));
