...
template<class A, class B> PX_FORCE_INLINE A unionCast(B b)
{
	union AB
	{
		AB(B bb) 
...
{
	return unionCast<unsigned int, float>(x);
}
#else
...
{
	return unionCast<float, unsigned int>(x);
}
#else
