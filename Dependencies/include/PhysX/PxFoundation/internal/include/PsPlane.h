...
#ifndef PX_FOUNDATION_PXPLANE_H
#define PX_FOUNDATION_PXPLANE_H
...
public:
	...
	*/
	PX_CUDA_CALLABLE PX_FORCE_INLINE Plane(PxReal nx, PxReal ny, PxReal nz, PxReal distance)
		: n(nx, ny, nz)
		, d(distance)
	{
	}
	...
	\brief Constructor from a normal and a distance
	*/
	PX_CUDA_CALLABLE PX_FORCE_INLINE Plane(const PxVec3& normal, PxReal distance) 
		: n(normal)
		, d(distance)
	{
	}
	...
	*/
	PX_CUDA_CALLABLE PX_FORCE_INLINE Plane(const PxVec3& point, const PxVec3& normal)
		: n(normal)		
	...
	*/
	...
	PX_CUDA_CALLABLE PX_FORCE_INLINE Plane transform(const PxTransform &transform) const
	{
		PxVec3 transformedNormal = transform.rotate(n);
		return Plane(transformedNormal, d - transform.p.dot(transformedNormal));
	}
	...
	*/
	...
	PX_CUDA_CALLABLE PX_FORCE_INLINE Plane inverseTransform(const PxTransform &transform) const
	{
		PxVec3 transformedNormal = transform.rotateInv(n);
		return Plane(transformedNormal, d + transform.p.dot(n));
	}
