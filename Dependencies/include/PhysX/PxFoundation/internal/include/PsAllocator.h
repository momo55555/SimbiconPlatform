...
#define PX_ALLOC(n)             physx::shdfnd3::Allocator().allocate(n, __FILE__, __LINE__)
#define PX_ALLOC_TEMP(n)        PX_ALLOC(n)
#define PX_FREE(x)              physx::shdfnd3::Allocator().deallocate(x)
...
#define PX_NEW(T)               new(physx::shdfnd3::ReflectionAllocator<T>(), __FILE__, __LINE__) T
...
// aligned allocation
#define PX_ALIGNED16_ALLOC(n) 	physx::shdfnd3::AlignedAllocator<16>().allocate(n, __FILE__, __LINE__)
#define PX_ALIGNED16_FREE(x)	physx::shdfnd3::AlignedAllocator<16>().deallocate(x) 
...
template <typename T>
PX_INLINE void* operator new(size_t size, physx::shdfnd3::ReflectionAllocator<T> alloc, const char* fileName, typename physx::shdfnd3::EnableIfPod<T, int>::Type line)
{
...
template <typename T>
PX_INLINE void  operator delete(void* ptr, physx::shdfnd3::ReflectionAllocator<T> alloc, const char* fileName, typename physx::shdfnd3::EnableIfPod<T, int>::Type line)
{
